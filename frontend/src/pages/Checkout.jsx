  ...
  return (
    <div className="min-h-screen pt-20">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="text-2xl font-heading font-bold text-white mb-8">Checkout</h1>
        <div className="flex items-center gap-4 mb-8">
          {[{ n: 1, label: 'Shipping' }, { n: 2, label: 'Review & Pay' }].map(({ n, label }) => (
            <div key={n} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${step >= n ? 'bg-primary-600 text-white shadow-glow' : 'bg-dark-700 text-dark-400'}`}>
                {step > n ? <CheckCircle className="w-4 h-4" /> : n}
              </div>
              <span className={`text-sm font-medium ${step >= n ? 'text-white' : 'text-dark-500'}`}>{label}</span>
              {n < 2 && <div className={`w-16 h-px ${step > n ? 'bg-primary-600' : 'bg-dark-700'}`} />}
            </div>
          ))}
        </div>
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            {step === 1 && (
              <div className="card p-6 animate-fade-in">
                <h2 className="text-lg font-heading font-semibold text-white mb-5 flex items-center gap-2">
                  <MapPin className="w-5 h-5 text-primary-400" /> Shipping Address
                </h2>
              ...
              <div className="space-y-5 animate-fade-in">
                <div className="card p-5">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-semibold text-white flex items-center gap-2"><MapPin className="w-4 h-4 text-primary-400" /> Delivery Address</h3>
                    <button onClick={() => setStep(1)} className="text-sm text-primary-400 hover:text-primary-300">Edit</button>
                  </div>
                ...
                <div className="card p-5">
                  <h3 className="font-semibold text-white mb-3">Order Items ({items.length})</h3>
                  <div className="space-y-2">
                    {items.map(item => (
                      <div key={item.id} className="flex justify-between text-sm">
                        <span className="text-dark-300">{item.product?.name} × {item.quantity}</span>
                        <span className="text-white">₹{parseFloat(item.total_price).toLocaleString('en-IN')}</span>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="card p-5">
                  <h3 className="font-semibold text-white mb-3 flex items-center gap-2"><Tag className="w-4 h-4 text-amber-400" /> Coupon Code</h3>
          ...
          <div className="lg:w-72 flex-shrink-0">
            <div className="card p-5 sticky top-24 space-y-3">
              <h2 className="font-heading font-bold text-white">Summary</h2>
              <div className="space-y-1.5 text-sm">
                <div className="flex justify-between text-dark-300"><span>Subtotal</span><span>₹{subtotal.toLocaleString('en-IN')}</span></div>
                ...
                <div className="flex justify-between text-dark-300"><span>Shipping</span><span className={shipping === 0 ? 'text-green-400' : ''}>{shipping === 0 ? 'FREE' : `₹${shipping}`}</span></div>
                <div className="border-t border-dark-700 pt-2 flex justify-between font-bold text-white text-base"><span>Total</span><span>₹{total.toLocaleString('en-IN')}</span></div>
              </div>
            </div>
          </div>
        </div>
